...
macro class Freezer implements ClassDeclarationsMacro {
  ...
  Future<void> buildDeclarationsForClass(
    ...
    for (final field in fields) {
      ...
      if (_collectionTypes.contains(fieldType)) {
        final generics =
            (field.type.code as NamedTypeAnnotationCode).typeArguments
    ...
    final fieldTypesWithGenerics = fieldTypes.map((name, type) {
    ...
    final code = [
      ...
      for (final fieldName in fieldNames) ...[
        if (_baseTypes.contains(fieldTypes[fieldName])) ...[
          "$fieldName: json['$fieldName'] as ${fieldTypes[fieldName]},".indent(
            6,
          ),
  ...
  void _buildHashCode(
    ...
    final code = [
      ...
      for (final fieldName in fieldNames) ...['$fieldName,'.indent(6)],
      ');'.indent(4),
      '}'.indent(2),
    ].join('\n');
    builder.declareInType(DeclarationCode.fromString(code));
